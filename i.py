...
filename = "Gays.txt"
Gays = open(filename, "a")
xss_log_file = "Gays.txt"
Gays2 = open(filename, "a")
finallist = []
threads = []
vuln = []
...
site = ['asia','ph','com','edu','gov','xxx','jobs','mobi','name','travel','ac', 'ad', 'ae','af', 'ag', 'ai', 'al', 'am', 'an', 'ao',
...
def cek():
	...
	for url in finallist:
		print (R+"[!] "+W+"SCANNING VULN,,...\r"),;sys.stdout.flush()
		EXT = "'"
		host = url+EXT
		try:
...
inurl = raw_input(R+"inurl"+G+" [?]"+W+": ")
site  = raw_input(R+"Site.."+G+" [?]"+W+": ")
maxc  = raw_input(R+"#-Pages"+G+"[?]"+W+": ")
